...
.hero-stat:hover{transform:translateY(-5px);border-color:rgba(124,92,255,.3)}
.hero-stat .num{font-family:'Space Grotesk',sans-serif;font-size:40px;font-weight:800;letter-spacing:-1.5px;margin-bottom:8px;color:#fff}
.hero-stat .lbl{font-size:13px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px}

.floating-icons{position:absolute;inset:0;pointer-events:none}
.float-icon{
  position:absolute;font-size:32px;opacity:.1;
  animation:floatUpDown 6s ease-in-out infinite;
}
.float-icon:nth-child(1){top:20%;left:8%;animation-delay:0s}
.float-icon:nth-child(2){top:70%;left:12%;animation-delay:-2s}
.float-icon:nth-child(3){top:15%;right:10%;animation-delay:-4s}
.float-icon:nth-child(4){top:65%;right:8%;animation-delay:-1s}
.float-icon:nth-child(5){top:45%;left:82%;animation-delay:-3s}
.float-icon:nth-child(6){top:80%;right:35%;animation-delay:-5s}
...
.feature-tag{
  ...
  color:var(--accent-2);background:rgba(34,211,238,.1);
  padding:4px 10px;border-radius:100px;
}

/* ============ MAPS & LOCAL SEO ============ */
.maps-grid{display:grid;grid-template-columns:1.2fr 1fr;gap:60px;align-items:center}
@media(max-width:960px){.maps-grid{grid-template-columns:1fr;gap:40px}}
.maps-points{margin:30px 0;list-style:none}
.maps-points li{display:flex;gap:12px;margin-bottom:16px;align-items:flex-start}
.maps-points li .check{color:var(--accent-3);font-weight:bold}
.maps-visual{
  position:relative;height:400px;border-radius:24px;
...
.map-blob{
  ...
  top:20%;left:25%;filter:blur(30px);
}
.map-search-bar{
  position:absolute;top:30px;left:30px;right:30px;
  padding:12px 20px;border-radius:100px;
  background:rgba(5,5,7,.8);border:1px solid var(--line);
...
.map-pin.p1{top:45%;left:45%;background:var(--grad);z-index:2;transform:rotate(-45deg) scale(1.3)}
.map-pin.p1::after{background:#fff}
.map-pin.p2{top:35%;left:25%;background:#555}
.map-pin.p3{top:65%;left:70%;background:#555}
.map-pin.p4{top:55%;left:15%;background:#555}

/* ============ AI SECTION ============ */
.ai-grid{display:grid;grid-template-columns:1fr 1.2fr;gap:60px;align-items:center}
@media(max-width:960px){.ai-grid{grid-template-columns:1fr;gap:40px}}
.chat-mock{
  border-radius:24px;background:#0b0b12;border:1px solid var(--line);
  padding:24px;height:400px;display:flex;flex-direction:col;gap:16px;
...
.chat-avatar{
  width:40px;height:40px;border-radius:50%;background:var(--grad-soft);
  display:flex;align-items:center;justify-content:center;font-size:20px;
}
.chat-name{font-weight:600;font-size:14px}
.chat-status{font-size:12px;color:var(--accent-3)}
.chat-msg{
  padding:12px 18px;border-radius:18px;max-width:85%;font-size:14px;
  animation:msgRise .4s cubic-bezier(.16,1,.3,1) forwards;opacity:0;
}
@keyframes msgRise{to{opacity:1;transform:translateY(0)}}
.chat-msg.bot{background:rgba(255,255,255,.04);color:#fff;align-self:flex-start;border-bottom-left-radius:4px}
.chat-msg.user{background:var(--grad);color:#fff;align-self:flex-end;border-bottom-right-radius:4px}
.chat-msg.m1{animation-delay:.5s}
.chat-msg.m2{animation-delay:2s}
.chat-msg.m3{animation-delay:3.5s}
.chat-msg.m4{animation-delay:5s}
.chat-msg.m5{animation-delay:6.5s}
...
.compare-row .col{font-size:15px}
.compare-row .col.bad{color:var(--muted)}
.compare-row .col.good{color:#fff;font-weight:600}
@media(max-width:760px){
  .compare-row{grid-template-columns:1fr;gap:8px}
  .compare-row.header{display:none}
  .compare-row .label{font-weight:bold;margin-bottom:4px;font-size:14px}
  .compare-row .col::before{content:attr(data-label) ": ";font-weight:500;color:var(--muted)}
}

/* ============ ROI CARDS ============ */
.roi-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:30px}
...
.roi-card .badge{
  ...
  font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:1px;
  color:var(--accent-3);background:rgba(16,185,129,.1);
  padding:4px 10px;border-radius:100px;
}
.roi-card .scenario{font-size:15px;color:var(--muted);margin-bottom:30px;padding-right:100px;line-height:1.5}
.roi-stat{
...
.roi-stat .v{font-weight:500;color:#fff}
.roi-card .result{
  margin-top:30px;padding-top:24px;border-top:1px dashed var(--line);
  display:flex;justify-content:space-between;align-items:center;
}
.roi-card .result .label{font-size:13px;text-transform:uppercase;letter-spacing:1px;color:var(--muted)}
.roi-card .result .value{font-family:'Space Grotesk',sans-serif;font-size:28px;font-weight:700;color:var(--accent-2)}

/* ============ PROCESS STEPS ============ */
.process-wrapper{position:relative;max-width:900px;margin:50px auto 0}
.process-wrapper::before{
  content:"";position:absolute;top:0;bottom:0;left:50%;width:1px;
  background:var(--line);transform:translateX(-50%);
}
@media(max-width:760px){.process-wrapper::before{left:30px}}
.process-step{
...
.step-number{
  width:44px;height:44px;border-radius:50%;background:#0b0b12;
  border:1px solid var(--accent);color:var(--accent);
  display:flex;align-items:center;justify-content:center;
  font-family:'Space Grotesk',sans-serif;font-weight:bold;font-size:18px;
  margin:0 auto;z-index:2;
  box-shadow:0 0 20px rgba(124,92,255,.1);
}
@media(max-width:760px){.step-number{margin:0}}

/* ============ FAQ LIST ============ */
.faq-list{max-width:800px;margin:50px auto 0;display:flex;flex-direction:col;gap:16px}
.faq-item{
  border-radius:18px;background:rgba(255,255,255,.01);
  border:1px solid var(--line);overflow:hidden;
  transition:all .3s ease;
}
.faq-item:hover{border-color:rgba(124,92,255,.2)}
.faq-q{
  width:100%;padding:24px 30px;text-align:left;background:none;border:none;
...
.faq-q .arrow{font-size:12px;color:var(--muted);transition:transform .3s ease}
.faq-a{
  max-height:0;overflow:hidden;padding:0 30px;
  transition:all .3s cubic-bezier(0,1,0,1);
}
.faq-a p{padding-bottom:24px;color:var(--muted);font-size:15px;line-height:1.7}
.faq-item.open{background:rgba(255,255,255,.02);border-color:rgba(124,92,255,.3)}
.faq-item.open .faq-q .arrow{transform:rotate(180deg)}
.faq-item.open .faq-a{max-height:1000px;transition:all .3s cubic-bezier(1,0,1,0)}

/* ============ FINAL CTA ============ */
.final-cta{
  text-align:center;padding:160px 5%;position:relative;
      ...
      <section className="hero">
        ...
        <div className="container">
          ...
          <div className="hero-cta-row">
            <button onClick={() => setIsModalOpen(true)} className="btn btn-primary">Get a Free Clinic Website Audit →</button>
        ...
        <div className="container">
          ...
          <div className="compare-table reveal">
            ...
            <div className="compare-row">
              ...
              <div className="col bad" data-label="Typical Agency">Just delivers a website</div>
              <div className="col good" data-label="21TechGlory">Builds a patient acquisition engine</div>
            </div>
            <div className="compare-row">
              <div className="label">Design</div>
              <div className="col bad" data-label="Typical Agency">Generic WordPress templates</div>
              <div className="col good" data-label="21TechGlory">Premium, custom medical UI</div>
            </div>
            <div className="compare-row">
              <div className="label">Appointment Booking</div>
              <div className="col bad" data-label="Typical Agency">Static contact form</div>
              <div className="col good" data-label="21TechGlory">Real-time booking + WhatsApp confirm</div>
            </div>
            <div className="compare-row">
              <div className="label">SEO</div>
              <div className="col bad" data-label="Typical Agency">Basic keywords once</div>
              <div className="col good" data-label="21TechGlory">Ongoing healthcare-specific SEO</div>
            </div>
            <div className="compare-row">
              <div className="label">Google Maps</div>
              <div className="col bad" data-label="Typical Agency">One-time GBP setup</div>
              <div className="col good" data-label="21TechGlory">Monthly Local Pack domination</div>
            </div>
            <div className="compare-row">
              <div className="label">Speed</div>
              <div className="col bad" data-label="Typical Agency">3–5s WordPress load</div>
              <div className="col good" data-label="21TechGlory">Sub-1s Next.js architecture</div>
            </div>
            <div className="compare-row">
              <div className="label">Automation</div>
              <div className="col bad" data-label="Typical Agency">None — manual everything</div>
              <div className="col good" data-label="21TechGlory">AI + WhatsApp + CRM end-to-end</div>
            </div>
            <div className="compare-row">
              <div className="label">ROI Reporting</div>
              <div className="col bad" data-label="Typical Agency">"Hope it works"</div>
              <div className="col good" data-label="21TechGlory">Monthly patient & revenue reports</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="final-cta" id="contact">
        <div className="container reveal">
          ...
          <div className="hero-cta-row" style={{"marginTop":"10px"}}>
            ...
            <a href="mailto:info@21techglory.com" className="btn btn-secondary">✉ Email Us</a>
          </div>
